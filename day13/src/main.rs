...
#....#..#";
    ...
    fn find_mirrors_with_n_diffs(input: &str, required_diffs: usize) -> usize {
        input
            .split("\n\n")
            .map(|pattern| {
                let pattern = pattern
                ...
                let potential_horizontal_mid = (0..pattern.len())
                    .tuple_windows()
                    .map(|(first, right)| {
                        (first, right, num_diffs(&pattern[first], &pattern[right]))
                    })
                ...
                for (first, second, diffs) in potential_horizontal_mid {
                    let total_diffs = std::iter::successors(
                        Some((first, second, diffs)),
                        |(first, second, _)| {
    ...
    println!("Part 1: {}", find_mirrors_with_n_diffs(input, 0));
    println!("Part 2: {}", find_mirrors_with_n_diffs(input, 1));
}
